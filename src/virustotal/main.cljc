(ns virustotal.main
  "Kotodama WASM entrypoint for the Virustotal clean-room actor (L4) — Clojure port.
  ...
  facts (`virustotal.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "virustotal")
(def tier "L4")
...
(def entity-specs
  [{:entity "Indicator"      :plural "indicators"      :id-prefix "virustot_ind"
    :fields [:type :value :confidence :firstSeen]     :required [:type :value]
    :coerce {:confidence :int}         :refs {}}
   {:entity "Vulnerability"  :plural "vulnerabilities" :id-prefix "virustot_vul"
    :fields [:cveId :cvss :description :published]    :required [:cvss :description]
    :coerce {:cvss :float}             :refs {}}
   {:entity "ThreatActor"    :plural "threatactors"    :id-prefix "virustot_thr"
    :fields [:name :motivation :sophistication]       :required [:name :motivation]
    :coerce {}                         :refs {}}
   {:entity "Campaign"       :plural "campaigns"       :id-prefix "virustot_cam"
    :fields [:name :actorId :firstSeen]               :required [:name :firstSeen]
    :coerce {}                         :refs {}}
   {:entity "Host"           :plural "hosts"           :id-prefix "virustot_hos"
    :fields [:ipAddress :asn :country :openPorts]     :required [:ipAddress :asn]
    :coerce {}                         :refs {}}
   {:entity "Technique"      :plural "techniques"      :id-prefix "virustot_tec"
    :fields [:attckId :tactic :name]                  :required [:tactic :name]
    :coerce {}                         :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"virustotal.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "virustotal-compat" :tier tier :entities entities} 200])
...
(defn start! [] :virustotal-compat/ready)
